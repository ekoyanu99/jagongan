...
describe('twitter', () => {
  ...
  describe('Initialize Tweet', () => {
    it('Should successfully initialize a tweet with valid topic and content', async () => {
      ...
      await program.methods
        .initialize(topic_bob1, content_bob1)
        .accounts({
    ...
    it('Should successfully initialize tweet with exactly 32-byte topic (boundary test)', async () => {
      ...
      await program.methods
        .initialize(topic_edge_case, content_bob1)
        .accounts({
          tweetAuthority: bob.publicKey,
          tweet: tweet_pkey,
          systemProgram: anchor.web3.SystemProgram.programId,
      ...
      await checkTweet(program, tweet_pkey, bob.publicKey, topic_edge_case, content_bob1, 0, 0, tweet_bump)
    })
    ...
    it('Should successfully initialize tweet with exactly 500-byte content (boundary test)', async () => {
      ...
      await program.methods
        .initialize('EdgeContent', content_edge_case)
        .accounts({
          tweetAuthority: bob.publicKey,
          tweet: tweet_pkey,
          systemProgram: anchor.web3.SystemProgram.programId,
      ...
      await checkTweet(program, tweet_pkey, bob.publicKey, 'EdgeContent', content_edge_case, 0, 0, tweet_bump)
    })
    ...
    it('Should successfully initialize tweet with empty content', async () => {
      ...
      await program.methods
        .initialize('EmptyContent', empty_content)
        .accounts({
          tweetAuthority: bob.publicKey,
          tweet: tweet_pkey,
          systemProgram: anchor.web3.SystemProgram.programId,
      ...
      await checkTweet(program, tweet_pkey, bob.publicKey, 'EmptyContent', empty_content, 0, 0, tweet_bump)
    })
    ...
    it('Should successfully initialize tweet with single character topic and content', async () => {
      ...
      await program.methods
        .initialize(single_char_topic, single_char_content)
        .accounts({
          tweetAuthority: bob.publicKey,
          tweet: tweet_pkey,
          systemProgram: anchor.web3.SystemProgram.programId,
      ...
      await checkTweet(program, tweet_pkey, bob.publicKey, single_char_topic, single_char_content, 0, 0, tweet_bump)
    })
    ...
    it('Should successfully initialize tweet with unicode characters and emojis', async () => {
      ...
      await program.methods
        .initialize(unicode_topic, unicode_content)
        .accounts({
          tweetAuthority: bob.publicKey,
          tweet: tweet_pkey,
          systemProgram: anchor.web3.SystemProgram.programId,
      ...
      await checkTweet(program, tweet_pkey, bob.publicKey, unicode_topic, unicode_content, 0, 0, tweet_bump)
    })
    ...
    it('Should fail to initialize tweet when topic exceeds 32 bytes', async () => {
      ...
      try {
        ...
        await program.methods
          .initialize(topic_bob2, content_bob2)
          .accounts({
            tweetAuthority: bob.publicKey,
            tweet: tweet_pkey,
            systemProgram: anchor.web3.SystemProgram.programId,
    ...
    it('Should fail to initialize tweet when content exceeds 500 bytes', async () => {
      ...
      try {
        ...
        await program.methods
          .initialize(topic_bob3, content_bob3)
          .accounts({
            tweetAuthority: bob.publicKey,
            tweet: tweet_pkey,
            systemProgram: anchor.web3.SystemProgram.programId,
    ...
    it('Should fail to initialize duplicate tweet with same topic and author', async () => {
      ...
      try {
        ...
        await program.methods
          .initialize(topic_bob1, 'Different content')
          .accounts({
            tweetAuthority: bob.publicKey,
            tweet: tweet_pkey,
            systemProgram: anchor.web3.SystemProgram.programId,
    ...
    it('Should allow different users to create tweets with same topic', async () => {
      ...
      await program.methods
        .initialize(topic_bob1, "Charlie's version")
        .accounts({
      ...
      await checkTweet(program, tweet_pkey, charlie.publicKey, topic_bob1, "Charlie's version", 0, 0, tweet_bump)
    })
  })
  ...
  describe('Add Comment', () => {
    ...
    it('Should successfully add comment with exactly 500 characters (boundary test)', async () => {
      ...
      await checkComment(program, comment_pkey, alice.publicKey, tweet_pkey, max_comment, comment_bump)
    })
    ...
    it('Should successfully add empty comment to tweet', async () => {
      ...
      await checkComment(program, comment_pkey, alice.publicKey, tweet_pkey, empty_comment, comment_bump)
    })
    ...
    it('Should successfully add comment with unicode characters and emojis', async () => {
      ...
      await checkComment(program, comment_pkey, alice.publicKey, tweet_pkey, unicode_comment, comment_bump)
    })
    ...
    it('Should allow multiple users to comment on the same tweet', async () => {
      ...
      await checkComment(program, comment_pkey, charlie.publicKey, tweet_pkey, charlie_comment, comment_bump)
    })
  ...
  describe('Tip Tweet', () => {
    ...
    beforeAll(async () => {
      ...
      await program.methods
        ...
        .accounts({
          tweetAuthority: author.publicKey,
          tweet: tweetPda,
          systemProgram: anchor.web3.SystemProgram.programId,
        })
        .signers([author])
...
function getCommentAddress(comment_content: string, author: PublicKey, parent_tweet: PublicKey, programID: PublicKey) {
  ...
  return PublicKey.findProgramAddressSync(
    [anchor.utils.bytes.utf8.encode(COMMENT_SEED), author.toBuffer(), content_seed, parent_tweet.toBuffer()],
    programID,
  )
}
...
function getTweetAddress(topic: string, author: PublicKey, programID: PublicKey) {
  return PublicKey.findProgramAddressSync(
    [anchor.utils.bytes.utf8.encode(topic), anchor.utils.bytes.utf8.encode(TWEET_SEED), author.toBuffer()],
    programID,
  )
}
...
function getReactionAddress(author: PublicKey, tweet: PublicKey, programID: PublicKey) {
  return PublicKey.findProgramAddressSync(
    [anchor.utils.bytes.utf8.encode(TWEET_REACTION), author.toBuffer(), tweet.toBuffer()],
    programID,
  )
}
...
async function checkTweet(
  ...
  if (tweet_author) {
    assert.strictEqual(
      tweetData.tweetAuthor.toString(),
      tweet_author.toString(),
      `Tweet author should be ${tweet_author.toString()} but was ${tweetData.tweetAuthor.toString()}`,
    )
  }
  if (topic) {
  ...
  if (content) {
    assert.strictEqual(
      tweetData.content,
      content,
      `Tweet content should be "${content}" but was "${tweetData.content}"`,
    )
  }
  if (likes || likes == 0) {
    assert.strictEqual(
      tweetData.likes.toString(),
      new anchor.BN(likes).toString(),
      `Tweet likes should be ${likes} but was ${tweetData.likes.toString()}`,
    )
  }
  if (dislikes || dislikes == 0) {
    assert.strictEqual(
      tweetData.dislikes.toString(),
      new anchor.BN(dislikes).toString(),
      `Tweet dislikes should be ${dislikes} but was ${tweetData.dislikes.toString()}`,
    )
  }
  if (bump) {
    assert.strictEqual(
      tweetData.bump.toString(),
      bump.toString(),
      `Tweet bump should be ${bump} but was ${tweetData.bump}`,
    )
  }
}
...
async function checkReaction(
  ...
  if (parent_tweet) {
    assert.strictEqual(
      reactionData.parentTweet.toString(),
      parent_tweet.toString(),
      `Reaction parent tweet should be ${parent_tweet.toString()} but was ${reactionData.parentTweet.toString()}`,
    )
  }
  if (bump) {
    assert.strictEqual(
...
async function checkComment(
  ...
  if (comment_author) {
    assert.strictEqual(
      commentnData.commentAuthor.toString(),
      comment_author.toString(),
      `Comment author should be ${comment_author.toString()} but was ${commentnData.commentAuthor.toString()}`,
    )
  }
  if (parent_tweet) {
    assert.strictEqual(
      commentnData.parentTweet.toString(),
      parent_tweet.toString(),
      `Comment parent tweet should be ${parent_tweet.toString()} but was ${commentnData.parentTweet.toString()}`,
    )
  }
  if (content) {
    assert.strictEqual(
      commentnData.content,
      content,
      `Comment content should be "${content}" but was "${commentnData.content}"`,
    )
  }
  if (bump) {
    assert.strictEqual(
      commentnData.bump.toString(),
      bump.toString(),
      `Comment bump should be ${bump} but was ${commentnData.bump}`,
    )
  }
}
